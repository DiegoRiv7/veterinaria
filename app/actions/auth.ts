...
export async function loginAction(_: unknown, formData: FormData): Promise<{ error?: string }> {
  ...
  if (phone.length < 7 || !password) {
    return { error: "Teléfono o contraseña inválidos." };
  }
...
export async function signupAction(
  ...
  if (!name || phone.length < 7 || password.length < 6) {
    return { error: "Revisa tus datos. La contraseña debe tener al menos 6 caracteres." };
  }
  const existing = await prisma.user.findUnique({ where: { phone } });
  if (existing) return { error: "Ya existe una cuenta con ese teléfono." };
